...
'''
    ...
    def edit_box(self):
        ...
        Label(self.edit_root, text='Amount Paid (Last Month)').grid(row=12, column=1, sticky=W)
        Entry(self.edit_root, textvariable=StringVar(self.edit_root, value=amt), state='readonly').grid(row=12,
                                                                                                        column=2)
        Label(self.edit_root, text='Amount Paid (This Month)').grid(row=13, column=1, sticky=W)
        new_amt = Entry(self.edit_root,textvariable=StringVar(self.edit_root, value=amt))
        new_amt.grid(row=13, column=2)

        self.test12 = StringVar(self.edit_root, value=new_amt)
        ...
        droplist.grid(row = 14,column =2, sticky= "e")

        Button(self.edit_root, text='Save Changes', command=lambda: self.edit_record(new_fname.get(), fname, new_father_name.get(), father_name, new_grandfather_name.get(), grandfather_name, new_address.get(), address, new_age.get(), age, new_contact.get(), contact, new_amt.get(),amt)).grid(row=14, column=1, sticky='w')
