...
class School_Portal:
    ...
    def __init__(self, root):
        self.root = root
        self.root.geometry('800x760+400+150')
       ...
       #Add Button
        ...
        ttk.Button(root, text='Print', command=self.show).grid(row=12, column=1, sticky = E)
...
#Attach files by dragging & dropping, selecting or pasting them.
    ...
    def search_rec(self):
        ...
        with sqlite3.connect(self.db_name) as conn:
            ...
            cursor.execute("SELECT * FROM studentlist WHERE Roll_No = ?",(self.rolln.get(),))
            d1=cursor.fetchall()
            ...
            cursor.execute("SELECT * FROM month WHERE Roll_No = ?",(self.rolln.get(),))
            d1=cursor.fetchall()
            out = [item for t in d1 for item in t]
            for i in range (0,14):
                Label(self.amountframe, text=str(out[i])+"  " ).grid(row=2, column=i, sticky=W)
            conn.commit()
            ttk.Button(self.valueframe,text='Print this record', command=self.show_specific).grid(row=0, column=2,sticky=E)
#Check if same month have previous data for money and send previous data
    def return_sum(self,stu_name,stu_fname,stu_gfname,stu_add,stu_age,stu_contact,stu_amount,mon):
        q='SELECT  Roll_No FROM studentlist WHERE Name=? AND Father_Name=? AND Grandfather_Name=? AND Address=? AND Age=? AND Contact=?'
        ...
        with sqlite3.connect(self.db_name) as conn:
            ...
            print("hok")
            print(mon)
            q1='SELECT  '+ mon +' FROM month WHERE Roll_No=?'
            p1=(dat1,)
            ...
            print("data is ")
            print(dat)
            q2= 'UPDATE month SET {0} = ? WHERE Roll_No = ?'.format(mon)
            p2=(dat,dat1)
    ...
    def run_query(self, query, parameters=()):
        with sqlite3.connect(self.db_name) as conn:
            ...
            cursor.execute('''CREATE TABLE IF NOT EXISTS studentlist
             ([Roll_No] INTEGER PRIMARY KEY,[Name] text,[Father_Name] text,[Grandfather_Name] text,[Address] text,[Age] integer , [Contact] text,[Amount] integer)''')
            cursor.execute('''CREATE TABLE IF NOT EXISTS month
              ([Roll_No] INTEGER PRIMARY KEY,[Baisakh] integer,[Jestha] integer,[Ashad] integer, [Shrawan] integer,[Bhadra] integer,[Ashwin] integer,[Kartik] integer,[Mangsir] integer,[Poush] integer,[Magh] integer,[Falgun] integer,[Chaitra] integer,[Total] integer)''')
    ...
    def return_row(self,id):
        with sqlite3.connect(self.db_name) as conn:
            cursor=conn.cursor()
            cursor.execute("SELECT * FROM month WHERE Roll_No = ?",(id,))
            d1=cursor.fetchall()
            #to list
            out = [item for t in d1 for item in t] 
            #print(d1)
            ...
            for i in range(1,13):
                ...
                #print (i)
            #print("HI"+str(totaln)) 
            cursor.execute('UPDATE month SET Total = ? WHERE Roll_No = ?',(totaln,id,))
            cursor.execute('UPDATE studentlist SET Amount = ? WHERE Roll_No = ?',(totaln,id,))
            cursor=conn.commit()
            #print(res) 
...
'''
    ...
    def add_record(self):
        if self.validation():
            q1= 'INSERT INTO month ( Roll_No,'+ self.test1.get()+') VALUES (?,?)'
    ...
    def delete_record(self):
        ...
        query = 'DELETE FROM studentlist WHERE Roll_No = ?'
        # Why comma
    ...
    #   print(new_amt)
        ...
        query = 'UPDATE studentlist SET Name=?, Father_Name=?, Grandfather_Name=?, Address=?, Age=?,  Contact=? WHERE Name=? AND Father_Name=? AND Grandfather_Name=? AND Address=? AND Age=? AND CONTACT=?'
    ...
    def show(self):
        ...
        df = pd.read_sql_query("""select * from {}""".format('studentlist'),conn)
        writer = pd.ExcelWriter('new2.xlsx', engine = 'xlsxwriter')
        ...
        app.Visible = False
        infile = os.path.join(os.path.abspath(SOURCE_DIR), 'new2.xlsx')
        outfile = os.path.join(os.path.abspath(TARGET_DIR), 'spreadsheet1.pdf')

        doc = app.Workbooks.Open(infile)
        doc.ExportAsFixedFormat(0, outfile)
        doc.Close()
        os.remove(infile)
        webbrowser.open_new(r'spreadsheet1.pdf')
    ...
    def show_specific(self):
        conn = sqlite3.connect('datas.db')
        rolll=self.rolln.get()
        df = pd.read_sql_query(('select * from "studentlist"  where "Roll_No" = :rol'),conn,params={"rol":rolll},index_col=['Roll_No'])
        writer = pd.ExcelWriter('new21.xlsx', engine = 'xlsxwriter')
        df.to_excel(writer,startrow = 1, sheet_name= 'Sheet1', index = False)
        workbook = writer.book
        worksheet = writer.sheets['Sheet1']
        #worksheet.set_column('A:A', 100)
        ...
        app = comtypes.client.CreateObject('Excel.Application')
        app.Visible = False
        infile = os.path.join(os.path.abspath(SOURCE_DIR), 'new21.xlsx')
        outfile = os.path.join(os.path.abspath(TARGET_DIR), 'spreadsheet2.pdf')

        doc = app.Workbooks.Open(infile)
        doc.ExportAsFixedFormat(0, outfile)
        doc.Close()
        os.remove(infile)
        webbrowser.open_new(r'spreadsheet2.pdf')
