...
class School_Portal:
    ...
    def __init__(self, root):
       ...
       #Add Button
        ...
        Chooser.add_command(label='Search')
...
'''
    ...
    def edit_box(self):
        ...
        subject = self.tree.item(self.tree.selection())['values'][5]
        ...
        Entry(self.edit_root, textvariable=StringVar(self.edit_root, value=subject), state='readonly').grid(row=12,
                                                                                                        column=2)
        ...
        print(subject)
        print("HI")
        print(self.new_contact.get())
        print("SHIT")
        print(self.amount.get())
        print(self.test12.get())
        Button(self.edit_root, text='Save Changes', command=lambda: self.edit_record(self.new_fname.get(), fname, self.new_fathername.get(), lname, self.new_grandfathername.get(), uname, self.new_address.get(), email,self.new_contact.get(), subject, self.new_age.get(), age, self.new_amount.get(), amount)).grid(row=16, column=2, sticky=W)

        self.edit_root.mainloop()


    def edit_record(self, new_finame, fname, new_fatname, lname, new_gfname, uname, new_add, email, new_cont, subject,
                    new_ag, age,new_amt,amt):
    	print("HI")
    	print(self.new_contact.get())
    	print("SHIT")
    	print(self.amount.get())
    	...
    	query = 'UPDATE studentlist SET Student_Name=?, Student_Father_Name=?, Student_Grandfather_Name=?, Address=?, Age=?,  Contact=? WHERE Student_Name=? AND Student_Father_Name=? AND Student_Grandfather_Name=? AND Address=? AND Age=?'
    	parameters = (new_finame, new_fatname, new_gfname, new_add, new_ag ,self.new_contact.get(), fname, lname, uname , email,age)
    	self.run_query(query, parameters)
    	print("Hi")
    	print(self.test12.get())
